...
manajemen_data_server <- function(id, values) {
    moduleServer(id, function(input, output, session) {
        ...
        observeEvent(input$apply_transform, {
            ...
            # Add the transformed variable to the dataset first
            temp_data <- values$sovi_data
            temp_data[[new_var_name]] <- transformed_values

            # Create result data frame with explicit column selection
            result_data <- data.frame(
                DISTRICTCODE = temp_data$DISTRICTCODE,
                original = temp_data[[var_name]],
                transformed = transformed_values
            )
            names(result_data)[2] <- var_name
            names(result_data)[3] <- new_var_name

            # Validate result_data structure
            if (ncol(result_data) != 3) {
                showNotification("Error: Struktur data hasil transformasi tidak valid.", type = "error")
                return()
            }
        ...
        output$transform_comparison <- renderPlotly({
            ...
            # Validate that transformed_data has at least 3 columns
            if (ncol(transformed_data()) < 3) {
                return(plotly_empty() %>% layout(title = "Error: Data tidak memiliki kolom yang cukup untuk perbandingan"))
            }
        ...
        output$transform_interpretation <- renderText({
            ...
            # Validate that transformed_data has at least 3 columns
            if (ncol(transformed_data()) < 3) {
                return("Error: Data transformasi tidak memiliki kolom yang cukup untuk interpretasi.")
            }
        ...
        observeEvent(input$create_derived, {
            ...
            derived_values <- tryCatch(
                {
                    if (input$derived_operation == "formula") {
                        ...
                        switch(input$derived_operation,
                            ...
                            "mean" = (var1 + var2) / 2,
                            # Default case
                            stop("Operasi tidak dikenali: ", input$derived_operation)
                        )
                    }
                },
                error = function(e) {
            ...
            showNotification(paste("Variabel", new_var_name, "berhasil dibuat!"), type = "message")
        })

        # Derived variable preview
        output$derived_preview <- renderPlotly({
            if (is.null(input$derived_var_name) || input$derived_var_name == "") {
                return(NULL)
            }
            ...
            tryCatch(
                {
                    ...
                    if (is.numeric(preview_values)) {
                        plot_ly(x = ~preview_values, type = "histogram") %>%
                            layout(
                                title = paste("Preview:", input$derived_var_name),
        ...
        observeEvent(input$handle_missing, {
            ...
            if (!is.null(processed_data)) {
                ...
                values$data_update_counter <- values$data_update_counter + 1
                showNotification("Penanganan missing data berhasil!", type = "message")
            }
        })
