...
if (!is.null(params$hasil_anova)) {
    # Convert ANOVA results to kable table
    ...
    ft <- knitr::kable(anova_df,
        col.names = c("Sumber Variasi", "df", "Sum of Squares", "Mean Square", "F-statistic", "p-value"),
        caption = "Tabel ANOVA"
    ) %>%
        kable_styling(
...
if (!is.null(params$posthoc_results)) {
    ...
    ft <- knitr::kable(posthoc_df,
        col.names = c("Perbandingan Grup", "Selisih Rata-rata", "CI Bawah", "CI Atas", "p-value (adjusted)"),
        caption = "Hasil Post-hoc Test (Tukey HSD)"
    ) %>%
        kable_styling(
            bootstrap_options = c("striped", "hover"),
            full_width = FALSE,
